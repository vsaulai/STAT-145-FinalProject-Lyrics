...
- Using a function, along with a set of code is used to webscrape for artist and song titles from wikipedia.
...
'udpipe uses the universal POS tags and are indicated below.
...
## Final Main Data Frame Created from Webscraping
  - Example of what the udpipe language model analyzed from our lyrics text for every song webscraped.

```{r, echo=FALSE}
head(lyrics_df[, 9:22], n = 5)
```
...
head(yearly_averages_rounded[, 1:8], n = 4)
```
...
```
## Parts of Speech(POS) Heat Map
Z-score is based off of a standard normal distribution
What the Z-score means from the heat map is if:
# Z-score > 0 :
...
- Proper Noun's i.e. names of Places, personal names, etc. were more significantly found in the top songs from 1958-1969. Colloquial references to other names were more popular in these songs than ever.
- As the decade's went by pronoun usage was low, but the average proportion of pronoun usage grew with time. This isn't a surprise because references to "He", "She", "it", "them", etc. are rather popular in romantic songs nowadays.
- Punctuations, often an indicator for slang like "Ain't", "'Til", "findin'" were more popular in the Top songs from 2020-2024 and the proportion of it were above the standard mean.
...
## Parts of Speech Word Type Proportion Among Top Songs Across the Decades.
...
## Verb Count per Year
- As with word count the amount of verb count per year increased since 1958 too.
