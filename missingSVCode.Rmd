...
scrape_genre_new_urls <- function(df, url_col, genre_col_name = "genre") {
  get_genre <- function(url) {
    tryCatch({
      webpage <- read_html(url)
      
      # Extract the genre node; the selector may need to be adjusted based on the actual page
      genre_node <- html_node(webpage, "p~ p+ p a")
      
      # Get text and clean it up
      genre <- html_text(genre_node)
      return(genre)
    }, error = function(e) {
      return(NA)
    })
  ...
  df[[genre_col_name]] <- sapply(df[[url_col]], get_genre)
...
```{r}
url <- final_df_artist_lyrics[["lyrics_url"]][1]
webpage <- read_html(url)
html_node(webpage, "p~ p+ p a") |> html_text()
```


## Run Genre Function
```{r}
genre_df_artist_lyrics <- scrape_genre_new_urls(final_df_artist_lyrics, "lyrics_url", genre_col_name = "genre")
View(genre_df_artist_lyrics)
```
...
## Regenerating New songlyrics URL

```{r}
url_missing_df_lyrics <- generate_songlyrics_urls(missing_df_lyrics, "Artist.s.", "Title", "https://www.songlyrics.com")
replacement_vector2 <- url_missing_df_lyrics$lyrics_url
missing_df_lyrics$lyrics_url[is.na(missing_df_lyrics$lyrics_url)] <- replacement_vector2 #Combine the Replacement Vector back into the larger data frame
```

## Run Scrape Function Attempt for larger missing lyric songs

```{r}
scrp_miss_df_lyrics <- scrape_lyrics_from_urls(missing_df_lyrics, "lyrics_url", lyrics_col_name = "lyrics")
View(scrp_miss_df_lyrics)
```

## Alternative Function: generate_azlyrics_url()
  - changing from 'songlyrics.com' to 'azlyrics.com'

```{r}
generate_azlyrics_urls <- function(df, artist_col, song_col) {
  # Format for AZLyrics URL: lowercase, remove features, remove special chars, no spaces
  format_for_azlyrics <- function(x) {
    x <- tolower(x)
    x <- gsub("\\s*(feat(uring)?|ft)\\.?\\s.*", "", x, ignore.case = TRUE)  # remove featuring info
    x <- gsub("[^a-z0-9]", "", x)  # remove all non-alphanumeric characters (no spaces, no hyphens)
    return(x)
  }
  ...
  # Generate URLs
  urls <- mapply(function(artist, song) {
    artist_clean <- format_for_azlyrics(artist)
    song_clean <- format_for_azlyrics(song)
    paste0(base_url, "/", artist_clean, "/", song_clean, ".html")
  }, df[[artist_col]], df[[song_col]])
